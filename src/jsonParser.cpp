...
void Parser::parse_value() {
    switch (*cur_) {
        case 'n' : parse_literal("null", Null); return;
        case 't' : parse_literal("true", True); return;
        case 'f' : parse_literal("false", False); return;
...
void Parser::parse_literal(const char *literal, type t){
...
void Parser::parse_string_raw(std::string &temp) {
    expect(cur_, '\"');
    const char *p = cur_;
    unsigned u = 0, u2 = 0;
    while (*p != '\"') {
