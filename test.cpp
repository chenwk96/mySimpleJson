...
static void test_parse_literal() {
    TEST_LITERAL(Null, "null");
    TEST_LITERAL(True, "true");
    TEST_LITERAL(False, "false");
}
...
#define TEST_NUMBER(expect, content)\
	do {\
		...
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(Number, j.get_type());\
...
#define TEST_STRING(expect, content)\
	do {\
		...
		v.parse(content, status);\
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(String, v.get_type());\
...
static void test_parse_array() {
	...
	v.parse("[ ]", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(Array, v.get_type());
	...
	EXPECT_EQ_BASE(Null,   v.get_array_element(0).get_type());
	EXPECT_EQ_BASE(False,  v.get_array_element(1).get_type());
	EXPECT_EQ_BASE(True,   v.get_array_element(2).get_type());
	EXPECT_EQ_BASE(Number, v.get_array_element(3).get_type());
	EXPECT_EQ_BASE(String, v.get_array_element(4).get_type());
	...
	for(int i = 0; i < 4; ++i) {
		...
		EXPECT_EQ_BASE(Array, a.get_type());
		EXPECT_EQ_BASE(i, a.get_array_size());
		for(int j = 0; j < i; ++j) {
            Json e = a.get_array_element(j);
			EXPECT_EQ_BASE(Number, e.get_type());
...
static void test_parse_object(){
	...
	v.parse(" { } ", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(Object, v.get_type());
	...
	EXPECT_EQ_BASE(Null, v.get_object_value(0).get_type());
	EXPECT_EQ_BASE("f", v.get_object_key(1));
	EXPECT_EQ_BASE(False, v.get_object_value(1).get_type());
	EXPECT_EQ_BASE("t", v.get_object_key(2));
	EXPECT_EQ_BASE(True, v.get_object_value(2).get_type());
	EXPECT_EQ_BASE("i", v.get_object_key(3));
	EXPECT_EQ_BASE(Number, v.get_object_value(3).get_type());
	EXPECT_EQ_BASE(123.0, v.get_object_value(3).get_number());
	EXPECT_EQ_BASE("s", v.get_object_key(4));
	EXPECT_EQ_BASE(String, v.get_object_value(4).get_type());
	EXPECT_EQ_BASE("abc", v.get_object_value(4).get_string());
	EXPECT_EQ_BASE("a", v.get_object_key(5));
	EXPECT_EQ_BASE(Array, v.get_object_value(5).get_type());
...
#define TEST_ERROR(error, content) \
	do {\
		...
		EXPECT_EQ_BASE((Null), v.get_type());\
	} while(0)
...
static void test_move() {
	...
	v2 = v1;
	v3 = std::move(v2);
	EXPECT_EQ_BASE(Null, v2.get_type());
...
static void test_access_boolean()
{
	...
	v.set_string("a");
	v.set_boolean(false);
	EXPECT_EQ_BASE(False, v.get_type());
}
